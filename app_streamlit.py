...
DATA_FILES = [
    ...
    "description.csv", "diets.csv",
    "medications.csv", "precautions.csv", "workout.csv",
]
MODEL_FILES = [
    "svc_model.pkl",
    "decision_tree_model.pkl", "label_encoder.pkl",
]
...
def load_data():
    main_df        = pd.read_csv("Diseases_and_Symptoms_dataset.csv")
...
if missing:
    ...
    st.markdown("""
**How to get them:**
1. Open your notebook in Google Colab and run the last cell that downloads the files.
2. Download: `svc_model.pkl`, `decision_tree_model.pkl`, 
   `label_encoder.pkl`, `Diseases_and_Symptoms_dataset.csv`, `description.csv`, 
   `diets.csv`, `medications.csv`, `precautions.csv`, `workout.csv`
3. Put them all in the same folder as `app.py`, then run:
   ```
